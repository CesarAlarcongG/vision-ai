...
import { parseCommand, VoiceCommand } from "@/services/voiceCommandService";
...
export function VoiceAssistantProvider({ children }: { children: ReactNode }) {
  ...
  const startListening = useCallback(async () => {
    ...
    if (startingRecognitionRef.current || recognitionRunningRef.current) {
      console.log("VOICE START CANCELLED: recognition already active");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.log("VOICE SERVICES CHECK ERROR:", serviceError);
      }
      ...
      if (!permission.granted) {
        ...
        speak("Necesito permiso para utilizar el micrófono.", voiceRateValue);
      ...
      speak("No pude iniciar el reconocimiento de voz.", voiceRateValue);
    } finally {
  ...
  const deactivateVoiceAssistant = useCallback(
    (announce: boolean = true) => {
      ...
      if (recognitionRunningRef.current) {
        ...
        console.log("VOICE: recognition was not running; abort skipped");
      }

      stopSpeaking();

      if (announce) {
        speak("Asistente de voz desactivado.", voiceRateValue);
      }
    },
    [voiceRateValue],
  );
    ...
    speak(
      ...
      voiceRateValue,
      {
        ...
        onDone: () => {
          ...
          setTimeout(() => {
            if (!activeRef.current) {
              console.log("VOICE START CANCELLED: assistant was disabled");
              return;
            }

            console.log("ASSISTANT: starting recognition after speech");
        ...
        onStopped: () => {
          console.log("ASSISTANT ACTIVATION SPEECH: stopped");
        },

        onError: (error) => {
          console.log("ASSISTANT ACTIVATION SPEECH ERROR:", error);
  ...
  }, [activateVoiceAssistant, deactivateVoiceAssistant]);

  const executeCommand = useCallback(
    (command: VoiceCommand) => {
      switch (command) {
        case "deactivate":
          respondAndContinue("El asistente se ha desactivado.");
          deactivateVoiceAssistant();
          return;
        ...
        case "settings":
          ...
          respondAndContinue("Abriendo la configuración.");
          return;

        case "results": // Cambiado de 'history' a 'results'
          router.replace("/results"); // Cambiado el path de '/history' a '/results'
          respondAndContinue("Abriendo los resultados.");
          return;
        ...
        case "back":
          if (pathname === "/home" || pathname === "/") {
            respondAndContinue("Ya te encuentras en la pantalla principal.");
          } else {
            router.back();
            respondAndContinue("Regresando a la pantalla anterior.");
          }
          return;
    ...
    [deactivateVoiceAssistant, pathname, respondAndContinue],
  );
  ...
  useSpeechRecognitionEvent("result", (event) => {
    ...
    console.log(event.isFinal ? "RAW TEXT:" : "RAW PARTIAL:", transcript);

    console.log("VOICE ALTERNATIVES:", event.results);

    if (!event.isFinal || !transcript || processingFinalResultRef.current) {
      return;
    }
  ...
  useSpeechRecognitionEvent("error", (event) => {
    ...
    if (event.error === "no-speech" || event.error === "speech-timeout") {
      pendingErrorRef.current = "No pude escucharte. Intenta nuevamente.";
    ...
    if (event.error === "not-allowed") {
      pendingErrorRef.current = "No tengo permiso para utilizar el micrófono.";
  ...
  useSpeechRecognitionEvent("end", () => {
    ...
    respondAndContinue("No pude identificar un comando. Intenta nuevamente.");
  });
  ...
  return (
    <VoiceAssistantContext.Provider value={contextValue}>
      ...
      <Modal
        ...
        onRequestClose={() => deactivateVoiceAssistant()}
      >
        <View style={styles.overlay}>
          <View style={styles.assistantCard}>
            ...
            <Text style={styles.description}>
              Di un comando como cámara, historial, configuración, inicio o
              desactivar voz.
            </Text>
            ...
            <Pressable
              ...
              onPress={() => deactivateVoiceAssistant()}
            >
              <Text style={styles.cancelText}>Desactivar voz</Text>
            </Pressable>
          </View>
        </View>
      </Modal>
